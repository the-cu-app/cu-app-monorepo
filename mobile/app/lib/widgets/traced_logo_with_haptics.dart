...
class TracedLogoWithHaptics extends StatefulWidget {
  final double width;
  final double height;
  final Color color;
  ...
  @override
  State<TracedLogoWithHaptics> createState() => _TracedLogoWithHapticsState();
}

class _TracedLogoWithHapticsState extends State<TracedLogoWithHaptics>
  ...
  void initState() {
    ...
    // Listen to trace animation for haptic feedback
    _traceController.addListener(_onTraceProgress);

    _startAnimationSequence();
  }

  void _onTraceProgress() async {
    final progress = _traceAnimation.value;

    // Only trigger haptics if progress has changed significantly (every 2%)
    if ((progress - _lastProgress).abs() > 0.02 && progress < 1.0) {
      _lastProgress = progress;

      // Calculate curvature-based intensity
      // Use sine wave to simulate varying curvature
      final curvatureFactor = (math.sin(progress * math.pi * 8) * 0.5) + 0.5;

      // Map curvature to haptic intensity
      // Light haptics at low curvature, heavy at high curvature
      final intensity = (curvatureFactor * 200).toInt().clamp(1, 255);

      if (await Vibration.hasVibrator() ?? false) {
        if (await Vibration.hasCustomVibrationsSupport() ?? false) {
          // Use custom vibration with varying intensity
          Vibration.vibrate(duration: 10, amplitude: intensity);
        } else {
  ...
  @override
  void dispose() {
    _traceController.removeListener(_onTraceProgress);
